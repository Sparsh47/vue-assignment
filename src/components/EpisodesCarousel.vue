...
<template>
  <div v-if="episodes.length" class="relative">
    <button
        @click="scroll('left')"
        class="absolute -left-8 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-20 backdrop-blur-sm w-12 h-12 rounded-full cursor-pointer hover:bg-red-500 hover:bg-opacity-80 duration-150 ease-in-out transition-all focus:outline-none flex items-center justify-center z-10"
    >
    ...
    >
      <div
          ...
          class="flex-shrink-0 w-80 cursor-pointer group rounded-lg"
          :class="{ 'border border-red-500': selectedEpisodeId === episode.id }"
      >
        <div class="relative h-48 rounded-lg overflow-hidden bg-gray-800">
          <img
              v-if="episode.image"
          ...
          <div class="absolute bottom-0 left-0 right-0 p-4 text-white">
            <div class="text-sm font-medium text-red-500 mb-1">
            ...
            <div v-if="episode.summary"
                 ...
                 v-html="episode.summary.replace(/<[^>]*>/g, '').substring(0, 120) + (episode.summary.length > 120 ? '...' : '')">
            </div>
          </div>
    ...
    <button
        @click="scroll('right')"
        class="absolute -right-8 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-20 backdrop-blur-sm w-12 h-12 rounded-full cursor-pointer hover:bg-red-500 hover:bg-opacity-80 duration-150 ease-in-out transition-all focus:outline-none flex items-center justify-center z-10"
    >
